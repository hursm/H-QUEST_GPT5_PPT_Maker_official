...
// H-QUEST Brand Colors
function getBrandColors() {
    // Check if dark mode is active (simple check based on body class or style)
    const isDarkMode = document.body.classList.contains('dark-mode') ||
        window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;

    return {
        primary: '#0477BF',       // Blue
        secondary: '#023373',     // Dark Blue
        accent: '#F2B705',        // Yellow/Gold
        neutral: isDarkMode ? '#E0E0E0' : '#333333',
...
function getChartColorPalette(count) {
    const baseColors = [
        '#0477BF', // Primary Blue
        '#F2B705', // Accent Yellow
        '#023373', // Dark Blue
        '#035AA6', // Medium Blue
        '#F29F05', // Orange-Yellow
        '#8C6900', // Dark Gold
        '#022859', // Very Dark Blue
        '#5BA4D9'  // Light Blue
    ];

    // If we need more colors, generate them or repeat
...
// General Chart Creation Function
function createChart(canvasId, config) {
    ...
    if (typeof Chart === 'undefined') {
        ...
        ctx.fillText('Chart.js library not loaded', 10, 50);
        return null;
    }

    // Apply common defaults if not specified
...
function createBarChart(canvasId, labels, datasets, title) {
    const colors = getBrandColors();
    const palette = getChartColorPalette(datasets.length);

    // Process datasets to ensure they have colors
    const processedDatasets = datasets.map((ds, index) => ({
        label: ds.label,
        data: ds.data,
        backgroundColor: ds.backgroundColor || palette[index],
        borderColor: ds.borderColor || palette[index],
        borderWidth: 1
    }));
...
function createLineChart(canvasId, labels, datasets, title) {
    ...
    const processedDatasets = datasets.map((ds, index) => ({
        ...
        data: ds.data,
        borderColor: ds.borderColor || palette[index],
        backgroundColor: ds.backgroundColor || (ds.fill ? `${palette[index]}33` : 'transparent'), // Add transparency if filled
        tension: 0.3,
        fill: !!ds.fill
    }));
...
function createDoughnutChart(canvasId, labels, data, backgroundColor, title) {
    const palette = backgroundColor || getChartColorPalette(data.length);
